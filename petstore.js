...
function calculateFoodOrder(numAnimals, avgFood) {
    ...
    if (!numAnimals || !avgFood || isNaN(Number(numAnimals)) || isNaN(Number(avgFood))) return -1;
...
function mostPopularDays(week) {
    ...
    week.forEach(weekDay => {
        if (weekDay.traffic > maxtraffic) maxtraffic = weekDay.traffic;
    });
    let maxDayObjetct = week.filter(weekDay => {
        return weekDay.traffic == maxtraffic;
    });
    return maxDayObjetct.map((maxDay) => {
        return maxDay.name;
    })
}
